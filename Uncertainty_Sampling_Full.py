...
file2 = plot_train_test(pl_train_results, pl_test_results, dataset_sizes,
                        ...
                        "PassiveLearningUS_Full_DiceScores", color_train="green", color_test="red")
...
def plot_combined_comparison(dataset_sizes, us_train, us_test, pl_train, pl_test):
    ...
    plt.grid(True)
    plt.tight_layout()
    filename = "ComparisonUS_Full_DiceScores.png"
    plt.savefig(filename, dpi=300)
    plt.close()
    print(f"Saved {filename}")
    return filename
...
save_df(pl_train_results, "PassiveLearningUS_Full_TrainDiceScores")
save_df(pl_test_results, "PassiveLearningUS_Full_TestDiceScores")
